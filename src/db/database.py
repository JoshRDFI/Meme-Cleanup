...
class DatabaseManager:
    ...
    def get_total_image_count(self) -> int:
        ...
        try:
            ...
            logger.error(f"Failed to get total image count: {e}")
            return 0
    
    def get_processed_image_count(self) -> int:
        """Get number of processed images (with embeddings)."""
        try:
            cursor = self._get_connection().cursor()
            cursor.execute("SELECT COUNT(*) FROM images WHERE clip_embedding IS NOT NULL")
            return cursor.fetchone()[0]
        except Exception as e:
            logger.error(f"Failed to get processed image count: {e}")
            return 0
    ...
    def get_duplicate_group_count(self) -> int:
        ...
        try:
            cursor = self._get_connection().cursor()
            cursor.execute("SELECT COUNT(DISTINCT group_id) FROM duplicate_groups")
            return cursor.fetchone()[0]
        except Exception as e:
            logger.error(f"Failed to get duplicate group count: {e}")
            return 0 
=======
"""
Database manager for Meme-Cleanup.
